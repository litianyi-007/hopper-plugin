...
const ENV_AFFECTING_ARGS = [
  ...
  // 2026-07-31: codex's sandbox-bypass default is now platform-split (see
  // codexSandboxBypassActive() in vendors/codex.js) and reversible per-shell via
  // this env var; neutralize it (and its sibling) so the generated matrix's
  // codex row reflects the PLATFORM split only, not whatever a generating
  // shell happens to have exported.
  'HOPPER_CODEX_SANDBOX_BYPASS', 'HOPPER_CODEX_SKIP_GIT_CHECK',
];
