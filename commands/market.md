...
One-shot market research via hopper's **ad-hoc** dispatch (no `queue.md` row). The `market-research` task-type auto-enables **web search** and *requests* a **read-only** sandbox, and routes to **codex** unless `--vendor` overrides. That read-only request is an instruction carried by the executor prompt frame, **not an OS-enforced boundary** — codex (the default) always runs full-access via `--dangerously-bypass-approvals-and-sandbox` (a deliberate Windows-sandbox workaround), regardless of the requested mode.
