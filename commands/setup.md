...
- **WebSrch** — `yes` means the adapter plumbs a web-search toggle headlessly (needed for research / market-research tasks): **codex, claude, grok, kimi**. `manual` (copilot, mimo) means the vendor can search but needs env/config; `no` (opencode, agy, pi) means unsupported out of the box — for **pi** specifically, its built-in tools are read/bash/edit/write/grep/find/ls with no web search, and hopper does not auto-install the extension that would add one.
...
- If **Vendor binaries** flags a vendor with several installs at different versions, treat that as the first thing to fix: hopper spawns whichever PATH resolves first, and PATH order differs between shells — so the version hopper runs need not be the one `<vendor> --version` shows you interactively. A stale shadowing install surfaces downstream as model rejections or protocol errors that look like account or capability problems. Run `hopper-dispatch --binaries --deep` for the absolute paths (kept out of `--setup`, which must stay path-free), then remove or repoint the shadowing entry.
