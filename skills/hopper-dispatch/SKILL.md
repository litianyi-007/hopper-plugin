...
8. `--vendor <name>` overrides the routed vendor for a REAL dispatch (sync, `--background`, `--adhoc`, `--swarm`) — it is validated against the registered adapters and the host/vendor family-separation guard still applies (see `cli/src/validation.js`'s `VENDOR_FAMILY`, not a literal string-equality check). **`--resolve` does not apply `--vendor`** — `--resolve <task-id> --vendor <v>` still prints the unmodified AGENTS.md/queue.md routing result, silently (known gap: `ISSUE-resolve-ignores-vendor-override.md`). Do not tell a user that `--resolve` previews an override — it does not.
