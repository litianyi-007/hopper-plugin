...
| 2026-07-31 | codex | Win vs macOS/Linux | **PARTIAL REVERSAL of the 2026-06-25 "codex has NO read-only scenario" row above** — that decision applied the Windows-only 1326 rationale to every platform. User-approved fix: `codexSandboxBypassActive(platform)` (`cli/src/vendors/codex.js`) now branches on `process.platform`. **Windows unchanged**: bypass stays the default (`-s` harness still cannot spawn children — 1326), `HOPPER_CODEX_SANDBOX_BYPASS=0` reverts to real `-s`. **macOS/Linux reversed**: bypass is now OFF by default — codex honors the requested `-s <mode>` for real, incl. read-only; `HOPPER_CODEX_SANDBOX_BYPASS=1` opts back into full-access bypass. Escape-hatch polarity is intentionally OPPOSITE per platform (documented in code + docs, not just this row). `--skip-git-repo-check` now rides EVERY mode/platform (previously bypass-path only) — a non-git CWD hits the SAME trust-gate error under a real `-s read-only` as under bypass (verified live, not assumed). `dispatch.js`'s `codexAlwaysFullAccess` and `setup.js`'s `sandboxControl()` both re-derive from the same platform-aware helper, so `doctor` sandbox col is now `full` on Windows / `argv` on macOS+Linux (was unconditionally `full`). `--subject-root`'s reachable surface widens: on macOS it now composes with a REAL codex read-only sandbox instead of being dead-on-arrival (codex's effective sandbox was always forced full-access before, so `--subject-root`'s `effective sandbox must be read-only` precondition could never hold for codex). | `[x]` shipped (v0.41.0) | **live** (macOS, this host): `codex exec -s read-only` denies a write (`operation not permitted`, file never created); `--dangerously-bypass-approvals-and-sandbox` with the same command creates it; `-s danger-full-access` (no bypass flag) also creates it (full-access parity preserved on POSIX, just via a different flag). `-s read-only` in a non-git dir hits the trust gate exactly like bypass did; `--skip-git-repo-check` clears it there too. Nested `sandbox-exec` (subject-root guard) wrapping `codex exec -s read-only` denies the write cleanly (no hang, no crash — Seatbelt nesting is fine). Unit: platform-injected fixtures (win32/darwin/linux) in codex-isolation.test.js, dispatch-flags.test.js, setup.test.js, vendor-security-claims.test.js, vendors-contract.test.js; destructive counter-proof pins the platform branch is load-bearing. |
